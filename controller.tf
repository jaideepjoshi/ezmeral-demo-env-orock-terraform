...
resource "openstack_blockstorage_volume_v3" "demo-controller-sda" {
  ...
  name        = "${var.prefix}-demo-controller-${count.index + 1}-sda"
  size        = 301
...
resource "openstack_blockstorage_volume_v3" "demo-controller-sdb" {
  count           = var.count-demo-controllers
  name        = "${var.prefix}-demo-controller-${count.index + 1}-sdb"
  size        = 501
  metadata = {
    User = var.openstack_username
  }
...
resource "openstack_blockstorage_volume_v3" "demo-controller-sdc" {
  count           = var.count-demo-controllers
  name        = "${var.prefix}-demo-controller-${count.index + 1}-sdc"
  size        = 501
  metadata = {
    User = var.openstack_username
  }
...
resource "openstack_compute_instance_v2" "demo-controller" {
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-controller-sda.*.id[count.index]}"
    source_type           = "volume"
    boot_index            = 0
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-controller-sdb.*.id[count.index]}"
    source_type           = "volume"
    boot_index            = 1
    destination_type      = "volume"
    delete_on_termination = true
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-controller-sdc.*.id[count.index]}"
    source_type           = "volume"
    boot_index            = 2
    destination_type      = "volume"
    delete_on_termination = true
