...
command -v openstack >/dev/null 2>&1  || { 
    echo >&2 "I require 'openstack' CLI but it's not installed.  Aborting."
    echo >&2 "Please install as per: https://https://docs.openstack.org/mitaka/user-guide/common/cli_install_openstack_command_line_clients.html"
    exit 1
}
...
command -v hpecp > /dev/null || {
    ...
    echo "Please install with: 'pip3 install --user --upgrade hpecp'or'pip3 install --user hpecp==v0.22.9'"
    exit 1
}
