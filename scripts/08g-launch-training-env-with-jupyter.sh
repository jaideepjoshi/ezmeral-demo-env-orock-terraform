...
./bin/ssh-dnsserver.sh rm -rf static/
./bin/ssh-dnsserver.sh mkdir static/

for FILE in $(ls -1 static/*)
do
  cat $FILE | ./bin/ssh-dnsserver.sh "cat > $FILE"
done
