...
echo "Copying hpecp.conf ro dnsserver"
scp -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" ./generated/hpecp.conf centos@${DNSSRVR_PRV_IP}:/home/centos/.hpecp.conf
...
ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${DNSSRVR_PRV_IP} "sudo chmod +w  /usr/local/share/man"
ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${DNSSRVR_PRV_IP} "sudo yum -y install perl-CPAN"
ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${DNSSRVR_PRV_IP} "export PERL_MM_USE_DEFAULT=1;perl -MCPAN -e 'install URI::Escape'"
