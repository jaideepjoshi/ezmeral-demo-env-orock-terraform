...
export AD_INSTANCE_ID=$(nova list |grep $PROJECT_ID|grep adserver | awk '{print $2}')
echo AD_INSTANCE_ID=$AD_INSTANCE_ID >> $env_variables
export AD_PRV_IP=$(nova list |grep $PROJECT_ID|grep adserver | awk '{ split($12, v, "="); print v[2]}')
echo AD_PRV_IP=$AD_PRV_IP >> $env_variables
export AD_PUB_IP=$(nova list |grep $PROJECT_ID|grep adserver | awk '{ split($12, v, "="); print v[2]}')
...
export DNSSRVR_INSTANCE_ID=$(nova list |grep $PROJECT_ID|grep dnsserver | awk '{print $2}')
echo DNSSRVR_INSTANCE_ID=$DNSSRVR_INSTANCE_ID >> $env_variables
export DNSSRVR_PRV_IP=$(nova list |grep $PROJECT_ID|grep dnsserver | awk '{ split($12, v, "="); print v[2]}')
echo DNSSRVR_PRV_IP=$DNSSRVR_PRV_IP >> $env_variables
export DNSSRVR_PUB_IP=$(nova list |grep $PROJECT_ID|grep dnsserver | awk '{ split($12, v, "="); print v[2]}')
...
export CTRL_INSTANCE_ID=$(nova list |grep $PROJECT_ID|grep controller | awk '{print $2}')
echo CTRL_INSTANCE_ID=$CTRL_INSTANCE_ID >> $env_variables
export CTRL_PRV_IP=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print v[2]}')
echo CTRL_PRV_IP=$CTRL_PRV_IP >> $env_variables
export CTRL_PUB_IP=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print v[2]}')
echo CTRL_PUB_IP=$CTRL_PRV_IP >> $env_variables
export CTRL_PRV_DNS=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print $4".demo.com"}')
echo CTRL_PRV_DNS=$CTRL_PRV_DNS >> $env_variables
export CTRL_PUB_DNS=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print $4".demo.com"}')
echo CTRL_PUB_DNS=$CTRL_PUB_DNS >> $env_variables
export CTRL_PUB_HOST=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print $4".demo.com"}')
echo CTRL_PUB_HOST=$CTRL_PUB_HOST >> $env_variables
export CTRL_PRV_HOST=$(nova list |grep $PROJECT_ID|grep controller | awk '{ split($12, v, "="); print $4".demo.com"}')
echo CTRL_PRV_HOST=$CTRL_PRV_HOST >> $env_variables

export GATW_INSTANCE_ID=$(nova list |grep $PROJECT_ID|grep gateway | awk '{print $2}')
echo GATW_INSTANCE_ID=$GATW_INSTANCE_ID >> $env_variables
export GATW_PRV_IP=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print v[2]}')
echo GATW_PRV_IP=$GATW_PRV_IP >> $env_variables
export GATW_PUB_IP=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print v[2]}')
echo GATW_PUB_IP=$GATW_PUB_IP >> $env_variables
export GATW_PRV_DNS=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print $4".demo.com"}')
echo GATW_PRV_DNS=$GATW_PRV_DNS >> $env_variables
export GATW_PUB_DNS=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print $4".demo.com"}')
echo GATW_PUB_DNS=$GATW_PUB_DNS >> $env_variables
export GATW_PUB_HOST=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print $4".demo.com"}')
echo GATW_PUB_HOST=$GATW_PUB_HOST >> $env_variables
export GATW_PRV_HOST=$(nova list |grep $PROJECT_ID|grep gateway | awk '{ split($12, v, "="); print $4".demo.com"}')
echo GATW_PRV_HOST=$GATW_PRV_HOST >> $env_variables

if [[ "$RDP_SERVER_ENABLED" == "True" ]]; then
   export RDP_INSTANCE_ID=$(nova list |grep $PROJECT_ID|grep rdpserver | awk '{print $1}')
   echo RDP_INSTANCE_ID=$RDP_INSTANCE_ID >> $env_variables
   export RDP_PRV_IP=$(nova list |grep $PROJECT_ID|grep rdpserver | awk '{ split($12, v, "="); print v[2]}')
   echo RDP_PRV_IP=$RDP_PRV_IP >> $env_variables
   export RDP_PUB_IP=$(nova list |grep $PROJECT_ID|grep rdpserver | awk '{ split($12, v, "="); print v[2]}')
...
if [[ "$WORKER_COUNT" != "0" ]]; then
   export WRKR_INSTANCE_IDS=$(nova list |grep $PROJECT_ID|grep k8s | awk '{ split($12, v, "="); print $2}'|awk 'BEGIN { ORS = " " } { print }')
   export WRKR_PRV_IPS=$(nova list |grep $PROJECT_ID|grep k8s | awk '{ split($12, v, "="); print v[2]}'|awk 'BEGIN { ORS = " " } { print }')
   export WRKR_PUB_IPS=$(nova list |grep $PROJECT_ID|grep k8s | awk '{ split($12, v, "="); print v[2]}'|awk 'BEGIN { ORS = " " } { print }')
...
export PICASSO_MASTER_HOSTS=$(nova list |grep $PROJECT_ID|grep k8sdfmlopsmaster | awk '{ split($12, v, "="); print v[2]}'|awk 'BEGIN { ORS = " " } { print }')
echo PICASSO_MASTER_HOSTS=${PICASSO_MASTER_HOSTS} >> $env_variables
export PICASSO_WORKER_HOSTS=$(nova list |grep $PROJECT_ID|grep k8sdfworker | awk '{ split($12, v, "="); print v[2]}'|awk 'BEGIN { ORS = " " } { print }')
echo PICASSO_WORKER_HOSTS=${PICASSO_WORKER_HOSTS} >> $env_variables
export MLOPS_WORKER_HOSTS=$(nova list |grep $PROJECT_ID|grep k8smlopsworker | awk '{ split($12, v, "="); print v[2]}'|awk 'BEGIN { ORS = " " } { print }')
echo MLOPS_WORKER_HOSTS=${MLOPS_WORKER_HOSTS} >> $env_variables

export PICASSO_MASTER_IDS=$(nova list |grep $PROJECT_ID|grep k8sdfmlopsmaster | awk '{ split($12, v, "="); print $2}'|awk 'BEGIN { ORS = " " } { print }')
echo PICASSO_MASTER_IDS=${PICASSO_MASTER_IDS} >> $env_variables
export PICASSO_WORKER_IDS=$(nova list |grep $PROJECT_ID|grep k8sdfworker | awk '{ split($12, v, "="); print $2}'|awk 'BEGIN { ORS = " " } { print }')
echo PICASSO_WORKER_IDS=${PICASSO_WORKER_IDS} >> $env_variables
export MLOPS_WORKER_IDS=$(nova list |grep $PROJECT_ID|grep k8smlopsworker | awk '{ split($12, v, "="); print $2}'|awk 'BEGIN { ORS = " " } { print }')
echo MLOPS_WORKER_IDS=${MLOPS_WORKER_IDS} >> $env_variables

#### EXTERNAL DF CLUSTER 1
