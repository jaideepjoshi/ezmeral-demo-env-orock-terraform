...
echo "The following instances have been created"
openstack server list --sort-column Name
echo "Waiting for 30s to give services a chance to startup"
sleep 30

echo "Establishing VPN connection to OpenVPN Server"
echo "Please provide your Macboock Password when Prompted"
nohup ./scripts/02a-start-vpn-connection.sh &
sleep 15

# test ssh connectivity
./scripts/02b-check-ssh-connections.sh

print_header "Installing AD Server"
./scripts/adserver-install.sh

print_header "Installing Controller"
./scripts/controller-install.sh
